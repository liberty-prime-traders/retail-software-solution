package me.ezra_home.retail_software_solution.util.model

import me.ezra_home.retail_software_solution.util.enums.HasCode
import me.ezra_home.retail_software_solution.util.enums.SchemaLevel

enum class TableName(override val code: String, val schemaLevel: SchemaLevel) : HasCode {

  // Platform-level tables
  AUTHORIZATION_PASS(TableNames.AUTHORIZATION_PASS, SchemaLevel.PLATFORM),
  DB_MIGRATION(TableNames.DB_MIGRATION, SchemaLevel.PLATFORM),
  DB_VERSION(TableNames.DB_VERSION, SchemaLevel.PLATFORM),
  FEATURE(TableNames.FEATURE, SchemaLevel.PLATFORM),
  IDENTITY_PROVIDER_LINK(TableNames.IDENTITY_PROVIDER_LINK, SchemaLevel.PLATFORM),
  JURISDICTION(TableNames.JURISDICTION, SchemaLevel.PLATFORM),
  JURISDICTION_TAX_TYPE(TableNames.JURISDICTION_TAX_TYPE, SchemaLevel.PLATFORM),
  JURISDICTION_TYPE(TableNames.JURISDICTION_TYPE, SchemaLevel.PLATFORM),
  ORGANIZATION(TableNames.ORGANIZATION, SchemaLevel.PLATFORM),
  ORGANIZATION_FEATURE(TableNames.ORGANIZATION_FEATURE, SchemaLevel.PLATFORM),
  PENDING_IDENTITY_LINK(TableNames.PENDING_IDENTITY_LINK, SchemaLevel.PLATFORM),
  PLATFORM_PERMISSION_ASSIGNMENT(TableNames.PLATFORM_PERMISSION_ASSIGNMENT, SchemaLevel.PLATFORM),
  PLATFORM_ROLE_ASSIGNMENT(TableNames.PLATFORM_ROLE_ASSIGNMENT, SchemaLevel.PLATFORM),
  RESERVED_SUBDOMAIN(TableNames.RESERVED_SUBDOMAIN, SchemaLevel.PLATFORM),
  SYS_USER(TableNames.SYS_USER, SchemaLevel.PLATFORM),
  TABLE_REGISTRY(TableNames.TABLE_REGISTRY, SchemaLevel.PLATFORM),
  TAX_TYPE(TableNames.TAX_TYPE, SchemaLevel.PLATFORM),

  // Organization-level tables
  ACCOUNT(TableNames.ACCOUNT, SchemaLevel.ORGANIZATION),
  ADDRESS(TableNames.ADDRESS, SchemaLevel.ORGANIZATION),
  ADJUSTMENT_REASON(TableNames.ADJUSTMENT_REASON, SchemaLevel.ORGANIZATION),
  CONTACT(TableNames.CONTACT, SchemaLevel.ORGANIZATION),
  EVENT_PROCESSING_LOG(TableNames.EVENT_PROCESSING_LOG, SchemaLevel.ORGANIZATION),
  FISCAL_PERIOD(TableNames.FISCAL_PERIOD, SchemaLevel.ORGANIZATION),
  JOB_TITLE(TableNames.JOB_TITLE, SchemaLevel.ORGANIZATION),
  LEDGER_ENTRY(TableNames.LEDGER_ENTRY, SchemaLevel.ORGANIZATION),
  LEDGER_ENTRY_GROUP(TableNames.LEDGER_ENTRY_GROUP, SchemaLevel.ORGANIZATION),
  LOCATION(TableNames.LOCATION, SchemaLevel.ORGANIZATION),
  OPENING_BALANCE(TableNames.OPENING_BALANCE, SchemaLevel.ORGANIZATION),
  OPENING_STOCK(TableNames.OPENING_STOCK, SchemaLevel.LOCATION),
  ORGANIZATION_ADMIN(TableNames.ORGANIZATION_ADMIN, SchemaLevel.ORGANIZATION),
  ORGANIZATION_JOIN_REQUEST(TableNames.ORGANIZATION_JOIN_REQUEST, SchemaLevel.ORGANIZATION),
  ORGANIZATION_USER(TableNames.ORGANIZATION_USER, SchemaLevel.ORGANIZATION),
  ORG_ACCOUNTING_CONFIG(TableNames.ORG_ACCOUNTING_CONFIG, SchemaLevel.ORGANIZATION),
  ORG_JURISDICTION_TAX_TYPE(TableNames.ORG_JURISDICTION_TAX_TYPE, SchemaLevel.ORGANIZATION),
  ORG_PERMISSION_ASSIGNMENT(TableNames.ORG_PERMISSION_ASSIGNMENT, SchemaLevel.ORGANIZATION),
  ORG_ROLE_ASSIGNMENT(TableNames.ORG_ROLE_ASSIGNMENT, SchemaLevel.ORGANIZATION),
  ORG_TABLE_REGISTRY(TableNames.ORG_TABLE_REGISTRY, SchemaLevel.ORGANIZATION),
  PAYMENT_METHOD(TableNames.PAYMENT_METHOD, SchemaLevel.ORGANIZATION),
  PRODUCT(TableNames.PRODUCT, SchemaLevel.ORGANIZATION),
  PRODUCT_CATEGORY(TableNames.PRODUCT_CATEGORY, SchemaLevel.ORGANIZATION),
  PRODUCT_GROUP(TableNames.PRODUCT_GROUP, SchemaLevel.ORGANIZATION),
  PRODUCT_TAG(TableNames.PRODUCT_TAG, SchemaLevel.ORGANIZATION),
  STOCK_ITEM_SOURCE(TableNames.STOCK_ITEM_SOURCE, SchemaLevel.ORGANIZATION),
  STOCK_MOVEMENT_REASON(TableNames.STOCK_MOVEMENT_REASON, SchemaLevel.ORGANIZATION),
  STOCK_MOVEMENT_REASON_TYPE(TableNames.STOCK_MOVEMENT_REASON_TYPE, SchemaLevel.ORGANIZATION),
  SUBLEDGER_ENTRY(TableNames.SUBLEDGER_ENTRY, SchemaLevel.ORGANIZATION),
  TAG(TableNames.TAG, SchemaLevel.ORGANIZATION),
  TAX_RATE(TableNames.TAX_RATE, SchemaLevel.ORGANIZATION),
  UNIT_CONVERSION(TableNames.UNIT_CONVERSION, SchemaLevel.ORGANIZATION),
  UNIT_GROUP(TableNames.UNIT_GROUP, SchemaLevel.ORGANIZATION),
  UNIT_VALUE(TableNames.UNIT_VALUE, SchemaLevel.ORGANIZATION),

  // Location-level tables
  LOCATION_PERMISSION_ASSIGNMENT(TableNames.LOCATION_PERMISSION_ASSIGNMENT, SchemaLevel.LOCATION),
  LOCATION_PRODUCT(TableNames.LOCATION_PRODUCT, SchemaLevel.LOCATION),
  LOCATION_ROLE_ASSIGNMENT(TableNames.LOCATION_ROLE_ASSIGNMENT, SchemaLevel.LOCATION),
  LOCATION_USER(TableNames.LOCATION_USER, SchemaLevel.LOCATION),
  PURCHASE(TableNames.PURCHASE, SchemaLevel.LOCATION),
  PURCHASE_DELIVERY(TableNames.PURCHASE_DELIVERY, SchemaLevel.LOCATION),
  PURCHASE_DELIVERY_LINE(TableNames.PURCHASE_DELIVERY_LINE, SchemaLevel.LOCATION),
  PURCHASE_LINE(TableNames.PURCHASE_LINE, SchemaLevel.LOCATION),
  SALE(TableNames.SALE, SchemaLevel.LOCATION),
  SALE_ADJUSTMENT(TableNames.SALE_ADJUSTMENT, SchemaLevel.LOCATION),
  SALE_LINE(TableNames.SALE_LINE, SchemaLevel.LOCATION),
  SALE_LINE_STOCK_RESERVATION(TableNames.SALE_LINE_STOCK_RESERVATION, SchemaLevel.LOCATION),
  SALE_PAYMENT(TableNames.SALE_PAYMENT, SchemaLevel.LOCATION),
  SALE_PAYMENT_VOID(TableNames.SALE_PAYMENT_VOID, SchemaLevel.LOCATION),
  SALE_VOID(TableNames.SALE_VOID, SchemaLevel.LOCATION),
  STOCK_ENTRY(TableNames.STOCK_ENTRY, SchemaLevel.LOCATION),
  STOCK_MOVEMENT(TableNames.STOCK_MOVEMENT, SchemaLevel.LOCATION),
  STOCK_TRANSFER_DISPATCH(TableNames.STOCK_TRANSFER_DISPATCH, SchemaLevel.LOCATION),
  STOCK_TRANSFER_DISPATCH_LINE(TableNames.STOCK_TRANSFER_DISPATCH_LINE, SchemaLevel.LOCATION),
  STOCK_TRANSFER_DRAFT_LINE(TableNames.STOCK_TRANSFER_DRAFT_LINE, SchemaLevel.LOCATION),
  STOCK_TRANSFER_ORDER(TableNames.STOCK_TRANSFER_ORDER, SchemaLevel.ORGANIZATION),
  STOCK_TRANSFER_RECEIPT(TableNames.STOCK_TRANSFER_RECEIPT, SchemaLevel.LOCATION),
  STOCK_TRANSFER_RECEIPT_LINE(TableNames.STOCK_TRANSFER_RECEIPT_LINE, SchemaLevel.LOCATION),
  SUPPLIER_PAYMENT(TableNames.SUPPLIER_PAYMENT, SchemaLevel.LOCATION),
  SUPPLIER_PAYMENT_VOID(TableNames.SUPPLIER_PAYMENT_VOID, SchemaLevel.LOCATION),
  SUPPLIER_RETURN(TableNames.SUPPLIER_RETURN, SchemaLevel.LOCATION),
  SYNC_LOG(TableNames.SYNC_LOG, SchemaLevel.LOCATION),
  TAX_ENTRY(TableNames.TAX_ENTRY, SchemaLevel.LOCATION),

  // Platform-level audit tables
  AUTHORIZATION_PASS_AUDIT(TableNames.AUTHORIZATION_PASS_AUDIT, SchemaLevel.PLATFORM),
  JURISDICTION_AUDIT(TableNames.JURISDICTION_AUDIT, SchemaLevel.PLATFORM),
  JURISDICTION_TAX_TYPE_AUDIT(TableNames.JURISDICTION_TAX_TYPE_AUDIT, SchemaLevel.PLATFORM),
  JURISDICTION_TYPE_AUDIT(TableNames.JURISDICTION_TYPE_AUDIT, SchemaLevel.PLATFORM),
  ORGANIZATION_AUDIT(TableNames.ORGANIZATION_AUDIT, SchemaLevel.PLATFORM),
  TAX_TYPE_AUDIT(TableNames.TAX_TYPE_AUDIT, SchemaLevel.PLATFORM),

  // Organization-level audit tables
  ACCOUNT_AUDIT(TableNames.ACCOUNT_AUDIT, SchemaLevel.ORGANIZATION),
  ADDRESS_AUDIT(TableNames.ADDRESS_AUDIT, SchemaLevel.ORGANIZATION),
  ADJUSTMENT_REASON_AUDIT(TableNames.ADJUSTMENT_REASON_AUDIT, SchemaLevel.ORGANIZATION),
  CONTACT_AUDIT(TableNames.CONTACT_AUDIT, SchemaLevel.ORGANIZATION),
  FISCAL_PERIOD_AUDIT(TableNames.FISCAL_PERIOD_AUDIT, SchemaLevel.ORGANIZATION),
  JOB_TITLE_AUDIT(TableNames.JOB_TITLE_AUDIT, SchemaLevel.ORGANIZATION),
  LOCATION_AUDIT(TableNames.LOCATION_AUDIT, SchemaLevel.ORGANIZATION),
  ORG_JURISDICTION_TAX_TYPE_AUDIT(TableNames.ORG_JURISDICTION_TAX_TYPE_AUDIT, SchemaLevel.ORGANIZATION),
  ORG_PERMISSION_ASSIGNMENT_AUDIT(TableNames.ORG_PERMISSION_ASSIGNMENT_AUDIT, SchemaLevel.ORGANIZATION),
  ORG_ROLE_ASSIGNMENT_AUDIT(TableNames.ORG_ROLE_ASSIGNMENT_AUDIT, SchemaLevel.ORGANIZATION),
  PAYMENT_METHOD_AUDIT(TableNames.PAYMENT_METHOD_AUDIT, SchemaLevel.ORGANIZATION),
  PRODUCT_AUDIT(TableNames.PRODUCT_AUDIT, SchemaLevel.ORGANIZATION),
  PRODUCT_CATEGORY_AUDIT(TableNames.PRODUCT_CATEGORY_AUDIT, SchemaLevel.ORGANIZATION),
  PRODUCT_GROUP_AUDIT(TableNames.PRODUCT_GROUP_AUDIT, SchemaLevel.ORGANIZATION),
  TAG_AUDIT(TableNames.TAG_AUDIT, SchemaLevel.ORGANIZATION),
  TAX_RATE_AUDIT(TableNames.TAX_RATE_AUDIT, SchemaLevel.ORGANIZATION),
  UNIT_CONVERSION_AUDIT(TableNames.UNIT_CONVERSION_AUDIT, SchemaLevel.ORGANIZATION),
  UNIT_GROUP_AUDIT(TableNames.UNIT_GROUP_AUDIT, SchemaLevel.ORGANIZATION),
  UNIT_VALUE_AUDIT(TableNames.UNIT_VALUE_AUDIT, SchemaLevel.ORGANIZATION),

  // Location-level audit tables
  LOCATION_PERMISSION_ASSIGNMENT_AUDIT(TableNames.LOCATION_PERMISSION_ASSIGNMENT_AUDIT, SchemaLevel.LOCATION),
  LOCATION_PRODUCT_AUDIT(TableNames.LOCATION_PRODUCT_AUDIT, SchemaLevel.LOCATION),
  LOCATION_ROLE_ASSIGNMENT_AUDIT(TableNames.LOCATION_ROLE_ASSIGNMENT_AUDIT, SchemaLevel.LOCATION),
  PURCHASE_AUDIT(TableNames.PURCHASE_AUDIT, SchemaLevel.LOCATION),
  PURCHASE_DELIVERY_AUDIT(TableNames.PURCHASE_DELIVERY_AUDIT, SchemaLevel.LOCATION),
  PURCHASE_DELIVERY_LINE_AUDIT(TableNames.PURCHASE_DELIVERY_LINE_AUDIT, SchemaLevel.LOCATION),
  PURCHASE_LINE_AUDIT(TableNames.PURCHASE_LINE_AUDIT, SchemaLevel.LOCATION),
  SALE_ADJUSTMENT_AUDIT(TableNames.SALE_ADJUSTMENT_AUDIT, SchemaLevel.LOCATION),
  SALE_AUDIT(TableNames.SALE_AUDIT, SchemaLevel.LOCATION),
  SALE_LINE_AUDIT(TableNames.SALE_LINE_AUDIT, SchemaLevel.LOCATION),
  STOCK_ENTRY_AUDIT(TableNames.STOCK_ENTRY_AUDIT, SchemaLevel.LOCATION),
  STOCK_MOVEMENT_AUDIT(TableNames.STOCK_MOVEMENT_AUDIT, SchemaLevel.LOCATION),
  STOCK_TRANSFER_DISPATCH_AUDIT(TableNames.STOCK_TRANSFER_DISPATCH_AUDIT, SchemaLevel.LOCATION),
  STOCK_TRANSFER_DISPATCH_LINE_AUDIT(TableNames.STOCK_TRANSFER_DISPATCH_LINE_AUDIT, SchemaLevel.LOCATION),
  STOCK_TRANSFER_DRAFT_LINE_AUDIT(TableNames.STOCK_TRANSFER_DRAFT_LINE_AUDIT, SchemaLevel.LOCATION),
  STOCK_TRANSFER_ORDER_AUDIT(TableNames.STOCK_TRANSFER_ORDER_AUDIT, SchemaLevel.ORGANIZATION),
  STOCK_TRANSFER_RECEIPT_AUDIT(TableNames.STOCK_TRANSFER_RECEIPT_AUDIT, SchemaLevel.LOCATION),
  STOCK_TRANSFER_RECEIPT_LINE_AUDIT(TableNames.STOCK_TRANSFER_RECEIPT_LINE_AUDIT, SchemaLevel.LOCATION),
  SUPPLIER_RETURN_AUDIT(TableNames.SUPPLIER_RETURN_AUDIT, SchemaLevel.LOCATION),
  TAX_ENTRY_AUDIT(TableNames.TAX_ENTRY_AUDIT, SchemaLevel.LOCATION);

  val tableName: String
    get() = code

  companion object {
    private val tableNameSet: Set<String> = entries.map { it.code }.toSet()
    fun exists(name: String?): Boolean = name in tableNameSet
  }
}
