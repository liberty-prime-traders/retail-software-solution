...
| `SaleSessionCommitHandler`     | `saveDraft`, `confirm` — server-side commit pipelines                |
...
SaleDraftCommitter  SaleConfirmCommitter (CONFIRMED branch:    (abandon)
...
| `originalStatus` | Lines / adjustments / header | Add payment                       | Remove payment              | Commit path            |
|------------------|------------------------------|-----------------------------------|-----------------------------|------------------------|
...
`SaleSessionValidator.canDiscardPayments` (which permits only DRAFT- or
CONFIRMED-loaded sessions) and drops it; when it's `persisted` the handler
...
UUID). Adjustments also carry a `lineIdentity: SessionIdentity?` pointing at
the line they're attached to (null = order-level).
...
- On commit:
  - `id != null`:
    - **lines** are updated in place — `SaleCommitLineSync` writes the
  ...
  - Adjustments missing from the input are deleted; payments missing from the
    input are rejected by `SalePaymentCommitter.ensureRemovalsRejected` (use
    `DELETE /payments` with `voidReason` to reverse a posted payment)
...
`saveDraft` (commit handler bumps both inside `applyOutcome` before saving).
...
| Save Draft     | `SaleDraftCommitter`   | No persisted-payment removals (`SalePaymentCommitter.ensureRemovalsRejected`); products still active; stock availability for requested quantities (`SaleValidator.guardStockForDraftUpdates`); if payments present, future-date guard + fiscal period open; new payments within payable total (`SalePaymentCommitter.guardWithinTotal`); optimistic `@Version` check on `SaleEntity` (existing sales only)         |
| Confirm        | `SaleConfirmCommitter` | At least one line; no persisted-payment removals (`SalePaymentCommitter.ensureRemovalsRejected`); products still active; future-date guard; fiscal period open; walk-in full coverage; payments within payable total; FIFO stock consumption; optimistic `@Version` check on `SaleEntity` (existing sales only)  |
...
4. SaleDraftCommitter.saveDraft:
     - reject any persisted payments missing from input
       (`SalePaymentCommitter.ensureRemovalsRejected`)
     ...
     - clear reservations for this sale, guard stock availability for the
       requested quantities (`guardStockForDraftUpdates`), re-reserve
     - sync adjustments (delete missing, insert new — existing kept as-is)
     - apply line + adjustment totals onto the sale (`SaleTotalsApplier`)
     - guard payments within payable total, append new payments
     - set `paymentStatus`, save sale
5. apply outcome to session: saleId, version, flip transient→id where
   applicable, bump `lastUpdatedAt` / `lastAccessedAt` / `lastAccessedById`
...
5. SaleConfirmCommitter.confirm:
     - guard ≥1 line, reject persisted-payment removals
       (`SalePaymentCommitter.ensureRemovalsRejected`), products active,
       date + fiscal period
     - load-or-create SaleEntity (`loadDraftAtVersion` for existing —
       rejects non-DRAFT or version mismatch; becomes CONFIRMED at end)
     - sync lines
     - acquire PRODUCT advisory lock, clear reservations
     - sync adjustments
     - apply line + adjustment totals onto the sale (`SaleTotalsApplier`)
     - guard within total, walk-in full-coverage (again, server-side belt
       and braces), append payments
     - set `paymentStatus`, flip status to CONFIRMED, save sale
     - run FIFO consumption via `SaleStockUpdater`
     - publish `SaleConfirmedEvent` via `SaleConfirmedHandlerForKafka`
6. apply outcome to session (new ids, bump `lastUpdated*` /
   `lastAccessed*`), build response, delete the session
```
...
- **Persisted payments cannot be silently dropped at commit time.**
  `SalePaymentCommitter.ensureRemovalsRejected` refuses a `saveDraft` /
  `confirm` whose input is missing a payment that exists in DB — that closes
  the "I forgot to echo this payment back" hole. Voiding through the
  `DELETE /payments` endpoint (with `voidReason`) is the correct way to
  reverse a posted payment; it bypasses the committer and goes straight
  through `SalePaymentService.voidPayment`.
