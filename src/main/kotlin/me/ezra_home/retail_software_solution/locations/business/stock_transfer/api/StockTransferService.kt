...
class StockTransferService(
    ...
    fun cancelTransfer(orderRef: String): StockTransferResponse {
        ...
        val dispatchRef = withLocationSchema(sourceSchema) { stockTransferSchemaGateway.cancelDispatch(orderRef) }
        ...
        return stockTransferResponseAssembler.build(stockTransferOrderService.getByReferenceNumber(orderRef))
    }

    fun fetchTop(limit: Int): List<StockTransferResponse> {
        val locationId = SessionContextProvider.getLocationId()
        return stockTransferOrderService.getTopByLocationId(locationId, limit)
