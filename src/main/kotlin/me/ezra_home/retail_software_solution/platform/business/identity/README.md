# Identity Package — Rules & Expectations

This document is the canonical reference for **how login and identity
linking work** in RTSS. It covers `platform/business/identity` and the
...
Read `cross_tier/authority/README.md` first if you haven't — login's
last step (`verifyRequestedRoles`) and every request after login
(`TenantFilter.refreshEffectiveRoles`) both go through
`EffectiveAuthorizationService` from that package.

## 1. Scope & Public Surface

```
platform/business/identity/       — identity-provider link + pending-link entities/repositories
platform/business/identity/api/   — public surface (IdentityService, LoginRequest/Response, IdentityProviderService)
```

| Class                        | Purpose                                                                                                     |
|------------------------------|-------------------------------------------------------------------------------------------------------------|
| `IdentityService`                | The whole login flow — resolve identity, mint session token, verify roles                                   |
...
`IdentityService.resolveSystemUserId` tries three things, in order:
...
`IdentityService.verifyRequestedRoles` filters `loginRequest.rolesToVerify`
...
`EffectiveAuthorizationService.getEffectiveRoles(systemUserId)` on every
