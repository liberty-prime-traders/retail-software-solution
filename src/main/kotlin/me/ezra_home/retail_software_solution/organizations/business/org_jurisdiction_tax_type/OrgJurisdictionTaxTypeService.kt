...
class OrgJurisdictionTaxTypeService(
    ...
    fun createAll(dtos: List<OrgJurisdictionTaxTypeInsertDto>): List<OrgJurisdictionTaxTypeResponseDto> {
        ...
        return entities.map { toResponseDto(it, index) }
    }
    ...
    fun update(dto: OrgJurisdictionTaxTypeUpdateDto): OrgJurisdictionTaxTypeResponseDto {
        ...
        return toResponseDto(entity, resolver.buildIndex())
    }

    private fun toResponseDto(entity: OrgJurisdictionTaxTypeEntity, index: Map<UUID, PlatformTaxTypeDto>): OrgJurisdictionTaxTypeResponseDto {
        val platformTaxType = index[entity.jurisdictionTaxTypeId]
            ?: throw RtsGenericException("Jurisdiction tax type not found: ${entity.jurisdictionTaxTypeId}")
        return orgJurisdictionTaxTypeMapper.toResponseDto(entity, platformTaxType)
    }
}
