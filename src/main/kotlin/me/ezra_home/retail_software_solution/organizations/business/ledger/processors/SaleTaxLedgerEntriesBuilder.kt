...
class SaleTaxLedgerEntriesBuilder(
    ...
    fun buildTransactionLevelEntries(
        eventDate: LocalDate,
        amountToTax: BigDecimal
    ): List<LedgerEntryRequest> = buildEntries(eventDate, amountToTax) { taxAmount, payableAccountCode ->
        listOf(
            LedgerEntryRequest(SystemAccount.GROSS_SALES.code, EntryType.DEBIT, taxAmount),
            LedgerEntryRequest(payableAccountCode, EntryType.CREDIT, taxAmount)
        )
    }

    fun buildTransactionLevelReversalEntries(
        eventDate: LocalDate,
        amountToTax: BigDecimal
    ): List<LedgerEntryRequest> = buildEntries(eventDate, amountToTax) { taxAmount, payableAccountCode ->
        listOf(
            LedgerEntryRequest(payableAccountCode, EntryType.DEBIT, taxAmount),
            LedgerEntryRequest(SystemAccount.GROSS_SALES.code, EntryType.CREDIT, taxAmount)
        )
    }
    ...
    private fun buildEntries(
        eventDate: LocalDate,
        amountToTax: BigDecimal,
        entriesFor: (taxAmount: BigDecimal, payableAccountCode: String) -> List<LedgerEntryRequest>
